...
public enum SimulatorOption {
    ...
    PEOPLE(Option.builder("people").desc("The number of people to simulate").hasArg().argName("num").build(),
                Integer.class, 10000),

    DURATION(Option.builder("duration").desc("The length of the simulation in hours").hasArg().argName("hours").build(),
    Integer.class, 24*14),

    DISTANCE(Option.builder("distance").desc("The maximum distance people travel in the grid").hasArg().argName("cells").build(),
    Integer.class, 100),

    CONTACT_PROB(Option.builder("contact").desc("The probability that a person meets another person in a given hour").hasArg().argName("prob").build(),
    Double.class, 0.1);
